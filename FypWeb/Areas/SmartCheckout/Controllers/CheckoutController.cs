                    ...
                    if (!string.IsNullOrEmpty(customerDetailsJson))
                    {
                        ...
                        try
                        {
                            ...
                            if (email != null)
                            {
                                ...
                                var subject = $"Order Receipt - Order #{orderHeader.Id}";
                                ...
                                foreach (var orderDetail in orderDetailsList)
                                {
                                    ...
                                    if (product != null)
                                    {
                                        orderDetailsHtml.Append($"<tr><td>{product.Name}</td><td>{orderDetail.Quantity}</td></tr>");
                                    }
                                }
                                var htmlBody = $@"
<html>
...
<body>
    <div class='container'>
        <div class='header'>Order Receipt - Order #{orderHeader.Id}</div>
        <div class='order-details'>
            <p>Your order has been successfully processed. Details:</p>
            <table>
                <tr>
                    <th>Product</th>
                    <th>Quantity</th>
        ...
        <div class='footer'>
            Total Amount: <strong>Rs.{orderHeader.OrderTotal}</strong>
        </div>
    </div>
</body>
</html>
";
                                ...
                                // Send the email with the constructed subject and HTML body
                                await SendEmailAsync(email, subject, htmlBody);
                            }

                            var productEntities = await _context.Product
            .Include(p => p.RecommendedProducts)
            .Include(p => p.ProductRecommendations) // Ensure to load the reverse recommendations
            .Where(p => productIds.Contains(p.Id))
            .ToListAsync();

                            // Collect all recommendations to remove, both where products are recommenders and recommended
                            var recommendationsToRemove = new List<ProductRecommendation>();
                            foreach (var product in productEntities)
                            {
                                recommendationsToRemove.AddRange(product.RecommendedProducts);
                                recommendationsToRemove.AddRange(product.ProductRecommendations);
                            }
                            // Remove the collected recommendations
                            _context.ProductRecommendations.RemoveRange(recommendationsToRemove);

                            // Remove the products
                            _context.Product.RemoveRange(productEntities);
        ...
        public async Task<IActionResult> RemoveProductsAndRecommendations(List<int> productIds)
        {
            var productEntities = await _context.Product
                .Include(p => p.RecommendedProducts)
                .Include(p => p.ProductRecommendations) // Include the reverse relationship
                .Where(p => productIds.Contains(p.Id))
                .ToListAsync();

            // Collect all recommendations to remove, both where products are recommenders and recommended
            var recommendationsToRemove = productEntities
                .SelectMany(p => p.RecommendedProducts)
                .Union(productEntities.SelectMany(p => p.ProductRecommendations))
                .ToList();

            _context.ProductRecommendations.RemoveRange(recommendationsToRemove);

            // Remove the products after all references in ProductRecommendations have been cleared
            _context.Product.RemoveRange(productEntities);

            try
            {
                await _context.SaveChangesAsync();
                return Json(new { success = true, message = "Products and recommendations removed successfully." });
            }
            catch (Exception ex)
            {
                // Log error or handle it as required
                return Json(new { success = false, message = $"Error removing products and recommendations: {ex.Message}" });
            }
        }
